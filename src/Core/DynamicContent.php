...
class DynamicContent
{
     ...
     */
    public function setStyle($styles = [], $pseudo_element = ''): string
    {
        $style_id = $this->getStyleID($pseudo_element);
        $this->styles[$style_id] = $styles;
        return !empty($pseudo_element) ? str_replace($pseudo_element, '', $style_id) : $style_id;
    }
     ...
     */
    public function getStyles($as_classes = true): string
    {
        $prefix = $as_classes ? '.' : '#';
        $styles_css = '';
    ...
    {
        $this->stylesHtml[$key] = (string)$value;
    }
     ...
     * @return string
     */
    public function getHtml($key = null): string
    {
        if (isset($this->stylesHtml[$key]) && !empty($key)) return $this->stylesHtml[$key];
        if (!isset($this->stylesHtml[$key]) && !empty($key)) return '';
        return implode('', $this->stylesHtml);
    }
     ...
     */
    private function getStyleID($pseudo_suffix = ''): string
    {
        $n = count($this->styles);
     ...
     * @return bool
     */
    private function isEmptyStyle($style_name = '', $style_value = ''): bool
    {
